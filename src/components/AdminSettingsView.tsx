...
export default function AdminSettingsView() {
  ...
  const [scanMode, setScanMode] = useState<string>("incremental");
  ...
  const handleScanBucket = async (libraryId?: number) => {
    ...
    setScanResults(null);
    try {
      const data = await scanBucket(token!, libraryId, scanMode);
      setScanResults(data);
  ...
  const handleScanArtistImages = async () => {
    ...
    try {
      ...
      const confirmed = window.confirm(`Êtes-vous sûr de vouloir supprimer la librairie '${name}' ? Cette action est irréversible.`);
      if (confirmed) {
        setLibLoading(true); // Use existing loading state for visual feedback
        setLibSuccess("");
        setLibError("");
        try {
          await deleteLibrary(token!, id);
          setLibSuccess(`Librairie '${name}' supprimée avec succès.`);
          setTimeout(() => setLibSuccess(""), 3000);
          loadLibraries(); // Refresh the list of libraries
        } catch (err: any) {
          setLibError(`Échec de la suppression de la librairie '${name}'.`);
        } finally {
          setLibLoading(false);
        }
      }
            ...
            {libLoading && libraries.length === 0 ? (
              ...
              libraries.map((lib, idx) => (
                <Card key={idx} className="border-border/40 shadow-sm overflow-hidden">
                  ...
                  <CardFooter className="bg-muted/5 border-t border-border/40 py-3 flex flex-wrap gap-4 items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Select onValueChange={setScanMode} defaultValue={scanMode}>
                        <SelectTrigger className="w-[180px] rounded-full h-8 text-xs">
                          <SelectValue placeholder="Mode de scan" />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="parquet">Parquet (Rapide)</SelectItem>
                          <SelectItem value="incremental">Scan Incrémental</SelectItem>
                          <SelectItem value="full">Scan Complet</SelectItem>
                        </SelectContent>
                      </Select>
                      <Button 
                        variant="outline"
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="flex flex-col gap-4">
                  <div className="space-y-2 max-w-xs">
                    <label className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Mode de Scan</label>
                    <Select onValueChange={setScanMode} defaultValue={scanMode}>
                      <SelectTrigger className="w-full rounded-full">
                        <SelectValue placeholder="Mode de scan" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="parquet">Récupération Rapide (Parquet)</SelectItem>
                        <SelectItem value="incremental">Scan Incrémental (Sync)</SelectItem>
                        <SelectItem value="full">Scan Complet (Metadata)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  {scanResults && (
                    <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg space-y-2 animate-in fade-in duration-300">
                      <p className="text-sm font-semibold flex items-center gap-2">
                        <Check className="w-4 h-4 text-green-500" /> {scanResults.message || "Scan terminé avec succès"}
                      </p>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                        <div className="p-2 bg-background rounded border border-border/40">
                          <span className="block font-bold text-lg">{scanResults.artists_scanned}</span>
                          <span className="text-muted-foreground">Artistes</span>
                        </div>
                        <div className="p-2 bg-background rounded border border-border/40">
                          <span className="block font-bold text-lg">{scanResults.albums_scanned}</span>
                          <span className="text-muted-foreground">Albums</span>
                        </div>
                        <div className="p-2 bg-background rounded border border-border/40">
                          <span className="block font-bold text-lg">{scanResults.tracks_scanned}</span>
                          <span className="text-muted-foreground">Titres</span>
                        </div>
                        <div className="p-2 bg-background rounded border border-border/40">
                          <span className="block font-bold text-lg text-destructive">{scanResults.tracks_removed}</span>
